...
{
    #region 新增
    public static float floor_scale=0.2f;//层的规模
    public static float point_scale = 0.5f;//引脚的规模
    private float floor_to_floor_width = 0.3f;//层与层之间距离
    public Transform init_pos;//生成点的位置
    ...
    {
        floor_count = uI_Control.net_floors_num;
        //更新
        basePanelPosition = uI_Control.init_baespos();
        basePanelPosition = basePanelPosition - new Vector3(0, floor_count * floor_to_floor_width / 2, 0);
        init_pos.transform.position = basePanelPosition;

        panelList[0].transform.position = basePanelPosition;
        vector3s[0] = basePanelPosition;
        
        for (int i = 0; i < floor_count - 1; i++)
        {
            Produce(maxpath_count - (int)((i + 1) / 3), i % 2 == 0 ? false : true, basePanelPosition + new Vector3(0, (i + 1) * floor_to_floor_width, 0), Get_Size(), ConstClass.nowfloor++);
        }
        isChange = true;
        panelList[0].GetComponent<SinglePanel>().IsSelet = false;
